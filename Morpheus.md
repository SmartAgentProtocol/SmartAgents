...
42 Million supply cap for the tokens. 14,400 tokens per day evenly distributed between users, capital, code, compute. 3,600 tokens for compute. Proof transactions for API calls served. 3,600 tokens for code. Proof code committed and merged into Morpheus repo. 3,600 tokens for capital. Proof ETH locked & yield donated to devs, compute and Users. 3,600 tokens for data. Proof of feedback on prompts, agents, and Morpheus.
...
As a result the Capital Provider will receive MTN tokens each day in pro-rata to their locked ETH. For example if there are 100 Capital Providers each Staking 1 ETH on day 1 when the network launches, then each one gets 1% of the 3,600 MTN tokens each day = 36 MTN.
